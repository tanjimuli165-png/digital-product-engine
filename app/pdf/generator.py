...
def generate_pdf(report: Report, output_path: Path) -> Path:
    ...
    for i, opp in enumerate(report.opportunities, 1):
        ...
        story += [table, Spacer(1, .16*inch), Paragraph("<b>Differentiation:</b> " + ptext("; ".join(opp.differentiation)), styles["Small"]), Paragraph("<b>Validation next:</b> " + ptext("; ".join(opp.next_steps)), styles["Small"]), Spacer(1, .15*inch)]
